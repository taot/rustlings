...
fn update_team_scores(scores: &mut HashMap<String, Team>, team_name: &str, goals_scored: u8, goals_conceded: u8) {
    ...
    //     },
    //     None => {
    //         let team = Team { goals_scored: goals_scored, goals_conceded: goals_conceded};
    //         scores.insert(team_name.to_owned(), team);
    //     }
    // }

    scores.entry(team_name.to_owned()).and_modify(|team| {
        team.goals_scored += goals_scored;
        team.goals_conceded += goals_conceded;
    }).or_insert(Team { goals_scored: goals_scored, goals_conceded: goals_conceded});
}
